      ...
      <main className="flex-1 pb-6 overflow-y-auto">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between mb-5">
          <div className="min-w-0 w-full">
            <h1 className="text-xl sm:text-2xl font-bold text-[#1e2040]">Proveedores</h1>
            <div className="mt-2 inline-flex w-full sm:w-auto rounded-xl border border-gray-200 bg-white p-1">
              <button
                type="button"
                onClick={() => setSection("suppliers")}
                className={`flex-1 sm:flex-initial px-3 py-2.5 sm:py-1.5 text-xs font-semibold rounded-lg min-h-[44px] sm:min-h-0 flex items-center justify-center ${section === "suppliers" ? "bg-[#4f6ef7] text-white" : "text-gray-500"}`}
              >
                Proveedores
              </button>
              <button
                ...
                className={`flex-1 sm:flex-initial px-3 py-2.5 sm:py-1.5 text-xs font-semibold rounded-lg min-h-[44px] sm:min-h-0 flex items-center justify-center ${section === "contacts" ? "bg-[#4f6ef7] text-white" : "text-gray-500"}`}
              >
                Contactos
              </button>
            </div>
          ...
          {isOwner ? (
            <button
              type="button"
              onClick={openCreate}
              className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-[#4f6ef7] text-white px-4 py-3 sm:py-2.5 text-sm font-semibold hover:bg-[#3d5ae0] shrink-0 min-h-[44px]"
            >
                    ...
                    {isOwner ? (
                      <div className="flex items-center gap-0.5 shrink-0">
                        <button
                          type="button"
                          onClick={() => openEdit(s)}
                          className="p-2.5 sm:p-1.5 rounded-lg text-gray-400 hover:bg-[#f0f3ff] hover:text-[#4f6ef7] min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 inline-flex items-center justify-center"
                          title="Editar"
                        >
                          <Pencil size={16} className="sm:w-[14px] sm:h-[14px]" />
                        </button>
                        <button
                          type="button"
                          onClick={async () => {
                          ...
                          className="p-2.5 sm:p-1.5 rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-500 disabled:opacity-50 min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 inline-flex items-center justify-center"
                          title="Desactivar"
                        >
